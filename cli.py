...
def run_triangle_simulation(args: argparse.Namespace) -> int:
    ...
    new_default_root = PROJECT_ROOT / "artefacts" / "run" / scenario_name
    output_dir = _resolve_output_directory(
        args.output_dir, new_default_root, timestamp_format="%Y%m%dT%H%M%SZ"
    )
    ...
    _safe_generate_debug_plots(output_dir)
    ...
    print(f"Formation window {duration:.1f} s. Artefacts written to {output_dir}")

    update_history_file(output_dir.name, str(config_path), args.notes)
    return 0
...
def setup_debug_logging():
    ...
    logging.basicConfig(
        level=logging.DEBUG, filename=LOG_PATH, filemode="a", encoding="utf-8",
...
def run_stub_simulation(args: argparse.Namespace) -> int:
    ...
    # This function has its own imports to keep it self-contained
    import matplotlib
    matplotlib.use("Agg")
    ...
    x_track, y_track = modulation * np.cos(angles), modulation * np.sin(angles)
    ...
    axis.legend()
    plot_path = output_dir / "formation_outline.png"
    ...
    record = {
        ...
        "artefacts": [plot_path.name], "status": "Stub simulation executed successfully.",
    }
    ...
    update_history_file(output_dir.name, "stub", args.notes)
    return 0
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser_triangle.add_argument(
        "--config", type=Path, default=DEFAULT_TRIANGLE_CONFIG,
        help="Path to the triangle formation configuration file.",
    )
    parser_triangle.add_argument(
        "--output-dir", type=Path, help="Directory in which to write simulation artefacts."
    )
    parser_triangle.add_argument(
        "--duration-days", type=float,
        help="Override the simulation duration in days.",
    )
    parser_triangle.add_argument("--notes", type=str, help="Notes to add to the history file.")
    ...
    parser_debug.add_argument(
        "--triangle-config", type=Path, default=DEFAULT_TRIANGLE_CONFIG,
        help="Override the triangle formation configuration file path.",
    )
    parser_debug.add_argument(
        "--output-root", type=Path, default=DEFAULT_DEBUG_ROOT,
    ...
    parser_debug.add_argument("--notes", type=str, help="Notes to add to the history file.")
    ...
    parser_stub.add_argument(
        "--output-dir", type=Path, help="Directory in which to store generated artefacts."
    )
    parser_stub.add_argument("--notes", type=str, help="Notes to add to the history file.")
