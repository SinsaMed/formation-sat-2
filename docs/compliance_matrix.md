...
| MR-2 | Mission Requirements | C | run_20251018_1937Z_tehran_daily_pass high-fidelity perturbation analysis[EV-5] | Retuned Plane B inclination and \(12°\) RAAN offset yield a \(0.13\,\text{km}\) node miss distance with Monte Carlo \(p_{95}=3.81\,\text{km}\), satisfying the ±10 km absolute criterion. |
...
| SRD-P-001 | System Requirements | C | run_20251018_1937Z_tehran_daily_pass high-fidelity perturbation analysis[EV-5] | Plane-intersection verification demonstrates \(0.13\,\text{km}\) deterministic separation and \(p_{95}=3.81\,\text{km}\) dispersion, closing the ±10 km requirement while documenting residual satellite-level offsets. |
...
| [EV-5] | run_20251018_1937Z_tehran_daily_pass high-fidelity perturbation analysis | `artefacts/run_20251018_1937Z_tehran_daily_pass` and `docs/tehran_daily_pass_scenario.md` | Captures deterministic node miss distance \(0.125\,\text{km}\), Monte Carlo \(p_{95}=3.81\,\text{km}\) intersection separation, solver settings, and refreshed STK exports generated via `sim/scripts/run_scenario.py` with \(J_2\)+drag.【Ref4】【Ref1】 |

Outstanding evidence actions: The quarterly rerun mandate has been satisfied by executing `run_20251018_1424Z`, which generated a drag-inclusive Monte Carlo catalogue using `sim/scripts/run_triangle_campaign.py` and archived outputs under `artefacts/run_20251018_1424Z`. The campaign history ledger (`artefacts/triangle_campaign/history.csv`) records the execution timestamp and schedules the next rerun for 16 January 2026, demonstrating that cadence governance is now automated. Atmospheric drag dispersions perturb the command geometry by \(p_{95} = 3.6\,\text{m}\) along-track while preserving the \(350\,\text{km}\) tolerance margin; the node-alignment requirement is now closed by [EV-5], so subsequent reruns should focus on regression rather than redesign.
