...
| [EV-4] | Tehran daily pass export package awaiting STK validation | `sim/scripts/run_scenario.py`, `config/scenarios/tehran_daily_pass.json`, and `docs/how_to_validate_tehran_stk.md` | Generates `.e`, `.sat`, `.gt`, `.fac`, and `.int` files under `run_YYYYMMDD_hhmmZ/stk_export`; STK 11.2 import and evidence capture to be recorded per the validation guide. |

Outstanding evidence actions: Maintain quarterly reruns of the `run_20251018_1207Z` workflow to monitor drift in command access geometry, extend the Monte Carlo campaign with atmospheric drag dispersion models, and refresh compliance statements if ground segment assumptions change. Complete the Tehran daily pass STK 11.2 import following `docs/how_to_validate_tehran_stk.md`, archive the resulting evidence under a new `run_` directory, and update the scenario metadata once validation is confirmed. Each activity must be logged with the corresponding run identifier and linked to automated tests where applicable.
