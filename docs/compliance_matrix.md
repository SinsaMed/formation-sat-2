...
| MR-2 | Mission Requirements | C | `artefacts/run_20260321_0740Z_tehran_daily_pass_locked` Tehran daily pass alignment analysis[EV-5] | Centroid cross-track of \(12.143\,\text{km}\) at 07:40:10Z with a worst-vehicle absolute offset of \(27.759\,\text{km}\) satisfies the ±30 km tolerance and preserves headroom beneath the ±70 km waiver ceiling, as confirmed by the locked deterministic summary. |
...
| SRD-P-001 | System Requirements | C | `artefacts/run_20260321_0740Z_tehran_daily_pass_locked` Tehran daily pass alignment analysis[EV-5] | Deterministic centroid cross-track remains at \(12.143\,\text{km}\) with a \(27.759\,\text{km}\) worst-vehicle offset at the midpoint, confirming compliance while Monte Carlo centroid statistics are scheduled for extension in the locked baseline. |
...
| [EV-3] | run_20251018_1207Z maintenance and responsiveness study covering MR-5 to MR-7 | `artefacts/run_20251018_1207Z` | Includes maintenance, command, and Monte Carlo CSV catalogues plus SVG CDF plot; supporting interpretation retained within `docs/triangle_formation_results.md`.[Ref7][Ref8] |
...
| [EV-5] | run_20260321_0740Z_tehran_daily_pass_locked Tehran daily pass alignment package (deterministic centroid metrics, Monte Carlo catalogue, STK exports) | `artefacts/run_20260321_0740Z_tehran_daily_pass_locked` | Captures the RAAN solver output, centroid cross-track compliance evidence, Monte Carlo catalogues, and refreshed STK exports produced by `sim/scripts/run_scenario.py` with \(J_2\)+drag perturbations, providing the locked baseline referenced in MR-2 and SRD-P-001.【Ref4】【Ref1】 |

Outstanding evidence actions: The quarterly rerun mandate has been satisfied by executing `run_20251018_1424Z`, which generated a drag-inclusive Monte Carlo catalogue using `sim/scripts/run_triangle_campaign.py` and archived outputs under `artefacts/run_20251018_1424Z`. The campaign history ledger (`artefacts/triangle_campaign/history.csv`) records the execution timestamp and schedules the next rerun for 16 January 2026, demonstrating that cadence governance is now automated. Atmospheric drag dispersions perturb the command geometry by \(p_{95} = 3.6\,\text{m}\) along-track while preserving the \(350\,\text{km}\) tolerance margin; forthcoming updates will extend Monte Carlo centroid statistics so that probabilistic statements accompany the deterministic compliance evidence supplied by `run_20260321_0740Z_tehran_daily_pass_locked`.
