...
| MR-2 | Mission Requirements | C | run_20251018_1936Z Tehran daily pass compliance package[EV-6] | Deterministic and Monte Carlo catalogues show ≤7.3 km minima with 100% fleet compliance, closing the prior non-conformance following the CCB clarification. |
...
| SRD-P-001 | System Requirements | C | run_20251018_1936Z Tehran daily pass compliance package[EV-6] | Updated propagation demonstrates ≤7.3 km minima for all planes with a fleet compliance probability of 1.0, satisfying the clarified acceptance criterion. |
...
| [EV-5] | run_20251018_1345Z high-fidelity perturbation analysis for the Tehran daily pass scenario (superseded) | `artefacts/run_20251018_1345Z` and `docs/tehran_daily_pass_scenario.md` | Retained for traceability; documents the pre-clarification geometry shortfall and solver configuration that triggered the CCB action.【Ref4】【Ref1】 |
| [EV-6] | run_20251018_1936Z Tehran daily pass compliance package with STK export | `artefacts/run_20251018_1936Z_tehran_daily_pass` and `artefacts/stk_run_20251018_1935Z` | Deterministic 3 s-sampled propagation with ten-run Monte Carlo dispersions plus refreshed STK exports; automated import deferred because `pywin32` is unavailable in the execution environment.【F:artefacts/run_20251018_1936Z_tehran_daily_pass/deterministic_summary.json†L1-L48】【F:artefacts/run_20251018_1936Z_tehran_daily_pass/monte_carlo_summary.json†L1-L77】【d52ff4†L1-L6】 |

Outstanding evidence actions: The quarterly rerun mandate has been satisfied by executing `run_20251018_1424Z`, which generated a drag-inclusive Monte Carlo catalogue using `sim/scripts/run_triangle_campaign.py` and archived outputs under `artefacts/run_20251018_1424Z`. The campaign history ledger (`artefacts/triangle_campaign/history.csv`) records the execution timestamp and schedules the next rerun for 16 January 2026, demonstrating that cadence governance is now automated. Atmospheric drag dispersions perturb the command geometry by \(p_{95} = 3.6\,\text{m}\) along-track while preserving the \(350\,\text{km}\) tolerance margin; with MR-2 and SRD-P-001 now closed via EV-6, the remaining open action is to exercise the updated STK package once a pywin32-enabled environment is available.
