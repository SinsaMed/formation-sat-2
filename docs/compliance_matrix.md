...
|----------------|-----------------|-------------------|--------------------|-----------------|
| MR-1 | Mission Requirements | C | `artefacts/run/tehran_triangle/triangle_run_20251114T000000Z/` triangular formation simulation summary[EV-1] | SERB confirmed plane allocation satisfies baseline geometry; continue to monitor during perturbation studies. |
| MR-2 | Mission Requirements | C | `artefacts/run/tehran_daily_pass_locked/20251020T190000Z/` Tehran daily pass alignment campaign[EV-5] | Deterministic midpoint centroid offset of \(12.143\,\text{km}\) with a \(27.759\,\text{km}\) worst vehicle remains within the ±30 km/±70 km thresholds, and the associated Monte Carlo catalogue reports centroid \(\vert x_{c}\vert\) statistics of \(23.914\,\text{km}\) (mean) and \(24.180\,\text{km}\) (\(p_{95}\)) with a \(39.761\,\text{km}\) \(p_{95}\) worst-spacecraft displacement, evidencing probabilistic compliance. |
| MR-3 | Mission Requirements | C | `artefacts/run/tehran_triangle/triangle_run_20251114T000000Z/` triangular formation simulation summary[EV-1] | Ninety-six second simultaneous access window verified; re-run once maintenance manoeuvre model is available. |
| MR-4 | Mission Requirements | C | `artefacts/run/tehran_triangle/triangle_run_20251114T000000Z/` triangular formation simulation summary[EV-1] | Triangle aspect ratio remains within ±2%; update tolerance check after incorporating sensor alignment effects. |
| MR-5 | Mission Requirements | C | `artefacts/run/unnamed_run/20251018T120700Z/` maintenance and responsiveness study[EV-3] | Single-station command latency measured at \(1.53\,\text{h}\) with \(10.47\,\text{h}\) margin; regression `tests/unit/test_triangle_formation.py` guards compliance. |
| MR-6 | Mission Requirements | C | `artefacts/run/unnamed_run/20251018T120700Z/` maintenance and responsiveness study[EV-3] | Weekly formation-keeping burns expend \(14.04\,\text{m/s}\) annually, preserving \(0.96\,\text{m/s}\) margin beneath the 15 m/s cap. |
| MR-7 | Mission Requirements | C | `artefacts/run/unnamed_run/20251018T120700Z/` maintenance and responsiveness study[EV-3] | Monte Carlo injection recovery succeeds in 300/300 trials with \(p_{95}\) \(\Delta v = 0.041\,\text{m/s}\), satisfying robustness expectations. |
| SRD-F-001 | System Requirements | C | `artefacts/run/tehran_triangle/triangle_run_20251114T000000Z/` triangular formation simulation summary[EV-1] | Deployment concept validated against mission plane allocation; confirm alongside launch vehicle ICD once drafted. |
| SRD-P-001 | System Requirements | C | `artefacts/run/tehran_daily_pass_locked/20251020T190000Z/` Tehran daily pass alignment campaign[EV-5] | Locked deterministic propagation yields \(12.143\,\text{km}\) centroid and \(27.759\,\text{km}\) worst-spacecraft offsets at the access midpoint, and the regenerated Monte Carlo set documents \(\bar{x}_{c}=23.914\,\text{km}\), \(p_{95}(\vert x_{c}\vert)=24.180\,\text{km}\), and \(p_{95}(\vert x\_{\mathrm{worst}}\vert)=39.761\,\text{km}\), demonstrating sustained compliance with the ±30 km/±70 km acceptance bands. |
| SRD-P-002 | System Requirements | C | `artefacts/run/tehran_triangle/triangle_run_20251114T000000Z/` triangular formation simulation summary[EV-1] | Access duration meets performance threshold; schedule regression once maintenance manoeuvres are synthesised. |
| SRD-P-003 | System Requirements | C | `artefacts/run/tehran_triangle/triangle_run_20251114T000000Z/` triangular formation simulation summary[EV-1] | Geometry tolerances satisfied; integrate sensor alignment error budget into next update. |
| SRD-O-001 | System Requirements | C | `artefacts/run/unnamed_run/20251018T120700Z/` maintenance and responsiveness study[EV-3] | Single-station command uplink achieved within \(1.53\,\text{h}\); latency ledger enforced via automated tests. |
| SRD-P-004 | System Requirements | C | `artefacts/run/unnamed_run/20251018T120700Z/` maintenance and responsiveness study[EV-3] | Annual \(\Delta v\) of \(14.04\,\text{m/s}\) meets propulsion budgeting requirement with documented weekly duty cycle. |
| SRD-R-001 | System Requirements | C | `artefacts/run/unnamed_run/20251018T120700Z/` maintenance and responsiveness study[EV-3] | Monte Carlo recovery catalogue evidences full success within \(15\,\text{m/s}\) per-spacecraft allocation. |
| SRD-O-002 | System Requirements | C | `artefacts/run/tehran_daily_pass/20251018T130800Z/` STK validation[EV-4] | STK 11.2 import confirmed imaging and downlink geometry; repeat after major configuration changes. |
...
|--------------|-------------|------------------|---------------------|
| [EV-1] | triangle_run triangular formation simulation package capturing ninety-six second Tehran access window | `artefacts/run/tehran_triangle/triangle_run_20251114T000000Z/` | JSON time-series under Git control; associated STK exports regenerated via `tools/stk_export.py` before acceptance.[Ref4][Ref6] |
...
| [EV-3] | run_20251018_1207Z maintenance and responsiveness study covering MR-5 to MR-7 | `artefacts/run/unnamed_run/20251018T120700Z/` | Includes maintenance, command, and Monte Carlo CSV catalogues plus SVG CDF plot; supporting interpretation retained within `docs/triangle_formation_results.md`. Windowed and full-propagation ground-distance maxima (\(343.62\,\text{km}\) and \(641.89\,\text{km}\)) are recorded explicitly to prevent ambiguity in compliance traceability.[Ref7][Ref8] |

| [EV-5] | run_20251020_1900Z_tehran_daily_pass_locked Tehran daily pass alignment package (deterministic centroid metrics, Monte Carlo catalogue, STK exports) | `artefacts/run/tehran_daily_pass_locked/20251020T190000Z/` | Records the post-fix RAAN solution, deterministic cross-track mid-point closure, Monte Carlo centroid distributions, and STK 11.2 exports generated via `sim/scripts/run_scenario.py` with \(J_2\)+drag, constituting the authoritative evidence set for MR-2 and SRD-P-001.【Ref4】【Ref1】 |

Outstanding evidence actions: The quarterly rerun mandate has been satisfied by executing the 2025-10-18 14:24Z run, which generated a drag-inclusive Monte Carlo catalogue using `sim/scripts/run_triangle_campaign.py` and archived outputs under `artefacts/run/tehran_triangle/20251018T142400Z/`. The campaign history ledger (`artefacts/history.csv`) records the execution timestamp and schedules the next rerun for 16 January 2026, demonstrating that cadence governance is now automated. Atmospheric drag dispersions perturb the command geometry by \(p_{95} = 3.6\,\text{m}\) along-track while preserving the \(350\,\text{km}\) tolerance margin; the regenerated Tehran daily pass baseline (`artefacts/run/tehran_daily_pass_locked/20251020T190000Z/`) now embeds Monte Carlo centroid statistics alongside the deterministic evidence, closing the previously deferred action.
